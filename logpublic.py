...
def log_building(log_name='runlog'):
    try:
        ...
        formatter = logging.Formatter(
            '[%(asctime)s][%(filename)s:%(lineno)d][%(levelname)s] %(message)s')  # [%(thread)d]
        ...
        trfilehandler = logging.handlers.TimedRotatingFileHandler("./logs/%s.log" % log_name, when='D', interval=1,
                                                                  backupCount=2)
        ...
        rfilehandler = logging.handlers.RotatingFileHandler("./logs/%s.log" % log_name, maxBytes=1024000000,
                                                            backupCount=5)
